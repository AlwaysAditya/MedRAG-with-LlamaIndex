...
- Clinical guideline PDF ingestion with LlamaParse
...
flowchart TB
  ...
  subgraph CORE["Domain-agnostic RAG core"]
    IDX["indexer.py"]
    RET["retriever.py"]
    GEN["generator.py"]
  end

  REG["projects.py<br/>project registry"]

  subgraph MED["MedRAG project plugin"]
...
```mermaid
flowchart LR
  A["Guideline PDFs"] --> C["MedRAGIngestor"]
  B["PubMed queries"] --> C
  C --> D["Parse and enrich metadata"]
  D --> E["Sentence chunking"]
  E --> F["FastEmbed vectors"]
...
- Docker with Compose
- OpenAI API key
- LlamaCloud API key for parsing guideline PDFs

```bash
cp .env.example .env
# Set OPENAI_API_KEY and LLAMA_CLOUD_API_KEY in .env
...
| `LLAMA_CLOUD_API_KEY` | - | Required for guideline PDF parsing |
